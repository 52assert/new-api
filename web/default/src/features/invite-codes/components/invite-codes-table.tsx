...
import { getInviteCodeStatusOptions } from '../constants'
...
function isDisabledInviteCodeRow(inviteCode: InviteCode) {
  return getInviteCodeStatusValue(inviteCode) !== '1'
}
...
export function InviteCodesTable() {
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      return (
        String(inviteCode.id).includes(keyword) ||
        String(inviteCode.code_prefix).toLowerCase().includes(keyword) ||
        String(inviteCode.remark || '')
          .toLowerCase()
          .includes(keyword)
      )
    },
  ...
  return (
    <DataTablePage
      ...
      toolbarProps={{
        searchPlaceholder: t('Filter by ID, prefix, or remark...'),
        filters: [
          {
            columnId: 'status',
            title: t('Status'),
